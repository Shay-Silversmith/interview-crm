-- =============================================================================
-- InterviewFlow — Initial Schema + RLS (IDEMPOTENT version)
-- Safe to run repeatedly: tables use IF NOT EXISTS, policies are dropped and
-- recreated cleanly each run.
--
...
-- interview_stages ----------------------------------------------------------
DROP POLICY IF EXISTS "Users manage own interview stages" ON interview_stages;
...
-- cv_versions ---------------------------------------------------------------
DROP POLICY IF EXISTS "Users manage own CV versions" ON cv_versions;
...
-- prepared_answers ----------------------------------------------------------
DROP POLICY IF EXISTS "Users manage own prepared answers" ON prepared_answers;
CREATE POLICY "Users manage own prepared answers"
  ON prepared_answers FOR ALL
  USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

-- ai_summaries --------------------------------------------------------------
DROP POLICY IF EXISTS "Users manage own AI summaries" ON ai_summaries;
CREATE POLICY "Users manage own AI summaries"
  ON ai_summaries FOR ALL
  USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

-- recent_activity -----------------------------------------------------------
DROP POLICY IF EXISTS "Users manage own activity" ON recent_activity;
...
CREATE INDEX IF NOT EXISTS interview_stages_app_id_idx  ON interview_stages(application_id);
...
CREATE INDEX IF NOT EXISTS recent_activity_created_idx  ON recent_activity(user_id, created_at DESC);
